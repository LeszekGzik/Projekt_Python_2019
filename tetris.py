	...
	def run(self):
		...
		while 1:
			...
			for event in pygame.event.get():
				if event.type == pygame.USEREVENT+1:    #info od Michała o wlaczeniu gry
					self.down()
				elif event.type == pygame.QUIT: #info od Michała o końcu gry
					sys.exit()
				elif event.type == pygame.KEYDOWN:  #info od Michała o wcisniętym klawiszu
					if event.key == eval("pygame.K_"+"LEFT"):
						self.slide(-1)
					if event.key == eval("pygame.K_"+"RIGHT"):
						self.slide(+1)
					if event.key == eval("pygame.K_"+"DOWN"):
						self.down()
					if event.key == eval("pygame.K_"+"UP"):
						self.rotate_cube()
					if event.key == eval("pygame.K_"+"p"):
